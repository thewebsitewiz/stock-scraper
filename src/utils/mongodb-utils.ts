...
import {
    Dividend, KeyExecutive, Notification, StockSymbolInfo
} from '../interfaces/stockSymbolInfo';
...
const dateFields = ["exOrEffDate", "declarationDate", "recordDate", "paymentDate"];
...
module.exports.insertSymbolData = async (data: StockSymbolInfo) => {
    ...
    if (!!data["exchange"] && !!data["symbol"]) {
        ...
        try {
            ...
            if (!!symbolResult) {
                ...
                if (notifications && notifications.length > 0) {
                    notifications.forEach(async (notification: any) => {
                        ...
                        notificationInsert["message"] = notification["eventTypes"][0]["message"];
                        notificationInsert["eventName"] = notification["eventTypes"][0]["eventName"];
                        notificationInsert["url"] = notification["eventTypes"][0]["url"]["value"];
                    ...
                    dividends.forEach(async (dividend: Dividend) => {
                        ...
                        dateFields.forEach(field => {
                            ...
                            if (dateIn === "N/A") {
                                ...
                                dividendInsert[field] = `${datePieces[2]}/${datePieces[0]}/${datePieces[1]}`;
                            }
                        });

                        dividendInsert["amount"] = Number(dividendInsert["amount"].replace("$", ""));
            ...
            console.log(`\n\n*********************************`);
            console.log(`Failed saveSymbolData: - ${err}\n${data}\n\n${saveSymbolData}\n\n`);
            console.log(`*********************************\n\n`);
